<x-app-layout title="antrian-bumil-vitamin">
    @push('css')
        ...
        <div class="col-12 mt-2">
            ...
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">
                        <div class="row">
                            <h4>Daftar antrian Pemberian vitamin pada Bumil</h4>
                        </div>
                    </h4>
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered zero-configuration">
                            ...
                            <tbody>
                                @foreach ($vitaminBumil as $vitamin)
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $vitamin->bumil->nama_ibu }}</td>
                                        <td>{{ $vitamin->bumil->nama_suami }}</td>
                                        <td>{{ Carbon\Carbon::parse($vitamin->bumil->tgl_lahir)->diff(\Carbon\Carbon::now())->format('%y thn') }}
                                        </td>
                                        <td> {{ Carbon\Carbon::parse($vitamin->bumil->tgl_kehamilan)->diffInWeeks(\Carbon\Carbon::now()) }}
                                            Minggu</td>
                                        <td>
                                            <x-action href="{{ route('vitaminbumil.edit', $vitamin->id) }}"
                                                action="{{ route('vitaminbumil.destroy', $vitamin->id) }}" />
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
